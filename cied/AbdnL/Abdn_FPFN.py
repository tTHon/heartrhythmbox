...
Threshold: prob > 0.30, pixel > 2800
"""
...
# ─── Load & prep ─────────────────────────────────────────────────────────────
res  = pd.read_csv("C:/CIEDID_data/AbdnL/FPFN/per_image_results.csv")
...
# Apply threshold: prob>0.30 AND pixel_count > 2800
res["pred_pos"] = (res["prob_30"] > 2800).astype(int)
...
print("=" * 65)
print("1. OVERALL CONFUSION MATRIX  (Test set n=50, threshold prob>0.30 & pixel>2800)")
print("=" * 65)
...
print(fn_cases[["img_id","Type","noActiveL","noAbdnL","targ_px","pred_px","prob_30",
...
print(fp_cases[["img_id","Type","noActiveL","pred_px","prob_30",
                 "Intracardiac device","Extracardiac type"]].to_string(index=False))
...
print("\n" + "=" * 65)
print("8. PIXEL COUNT ANALYSIS  (prob_30 = pixel count at prob>0.30)")
print("=" * 65)
print(f"\n{'Outcome':<6s}  {'n':>3s}  {'prob_30 mean':>13s}  {'prob_30 median':>14s}  {'targ_px mean':>13s}")
print("-" * 58)
for oc in ["TP","FP","FN","TN"]:
    g = merged[merged["outcome"]==oc]
    if len(g)==0: continue
    print(f"{oc:<6s}  {len(g):>3d}  {g['prob_30'].mean():>13.0f}  {g['prob_30'].median():>14.0f}  {g['targ_px'].mean():>13.0f}")

# FN: show individual prob_30 vs targ_px vs threshold
print(f"\nFN cases — pixel detail (threshold = 2800):")
print(fn_cases[["img_id","targ_px","pred_px","prob_30"]].to_string(index=False))