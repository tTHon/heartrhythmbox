...
def plot_samples(models, dls, device, n=6, output_path=None):
    imgs, gts, preds_list = [], [], []
    mean = torch.tensor([0.5026684403419495, 0.5026684403419495, 0.5026684403419495]).view(3,1,1)
    std  = torch.tensor([0.2409660518169403, 0.2409660518169403, 0.2409660518169403]).view(3,1,1)
...
def main():
    ...
    parser.add_argument("--thresholds",      type=int,   nargs="+",
                        default=[500, 750, 1000, 1250, 1500, 1750, 2000, 2250, 2500, 2750, 3000, 3250, 3500, 3750, 4000])
    parser.add_argument("--prob_thresholds", type=float, nargs="+",
                        default=[0.1, 0.2, 0.3, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.70, 0.75, 0.8, 0.85, 0.9, 0.95, 1.0])
