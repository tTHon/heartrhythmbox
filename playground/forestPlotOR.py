...
plt.figure(figsize=(40, 5))  # Adjust height for better visibility
y_pos = np.arange(3) 

# Plot Random Effects Models
plt.plot(surgical_random_effects["OR"], 0, 'D', color='darkorange', markersize=10)
plt.hlines(0, surgical_random_effects["CI_lower"], surgical_random_effects["CI_upper"], colors='darkorange', linewidth=10)

plt.plot(medical_random_effects["OR"], 1, 'D', color='royalblue', markersize=10)
plt.hlines(1, medical_random_effects["CI_lower"], medical_random_effects["CI_upper"], colors='royalblue', linewidth=10)

plt.plot(combined_random_effects["OR"], 2, 'D', color='forestgreen', markersize=10)
plt.hlines(2, combined_random_effects["CI_lower"], combined_random_effects["CI_upper"], colors='forestgreen', linewidth=10)
