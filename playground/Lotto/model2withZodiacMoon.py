...
class LotteryPredictor:
    def __init__(self):
        ...
        self.moon_phase_encoder = LabelEncoder()

    def load_data(self, csv_path, number_column='lottery_number', zodiac_column='zodiac', moon_phase_column='moonPhase'):
        try:
            data = pd.read_csv(csv_path)
            
            # Check required columns
            required_columns = [number_column, zodiac_column, moon_phase_column]
            ...
            data = data.dropna(subset=[number_column, zodiac_column, moon_phase_column])
            
            # Encode zodiac signs and moon phases
            data['zodiac_encoded'] = self.zodiac_encoder.fit_transform(data[zodiac_column])
            data['moon_phase_encoded'] = self.moon_phase_encoder.fit_transform(data[moon_phase_column])
    ...
    def prepare_data(self, historical_data, number_column='lottery_number', zodiac_column='zodiac_encoded', moon_phase_column='moon_phase_encoded', window_size=10):
        # Prepare number data
        ...
        # Prepare moon phase data
        moon_phases = historical_data[moon_phase_column].values
        ...
        for i in range(len(scaled_numbers) - window_size):
            # Combine number, zodiac, moon phase, and additional features
            number_window = scaled_numbers[i:i + window_size]
            zodiac_window = zodiacs[i:i + window_size]
            moon_phase_window = moon_phases[i:i + window_size]
            day_of_week_window = days_of_week[i:i + window_size]
            combined_features = np.column_stack((number_window, zodiac_window, moon_phase_window, day_of_week_window))
    ...
    def predict(self, recent_data, recent_zodiac, recent_moon_phase):
        if self.model is None:
            raise ValueError("Model has not been built or trained yet.")
        
        # Scale number and prepare zodiac and moon phase
        ...
        recent_moon_phase_encoded = self.moon_phase_encoder.transform([recent_moon_phase])[0]
        
        # Combine features
        combined_features = np.column_stack((recent_numbers, [recent_zodiac_encoded] * len(recent_numbers), [recent_moon_phase_encoded] * len(recent_numbers)))
...
def main():
    CSV_PATH = 'playground/Lotto/lottery_with_moon_phases.csv'
    ...
    try:
        ...
        predictor.build_lstm_model(input_shape=(X.shape[1], X.shape[2]))  # Update input shape to include moon phase
        ...
        recent_data = X_test[-1][:, 0]  # Extract the recent number data
        recent_zodiac = 'Gemini'  # example zodiac sign
        recent_moon_phase = 'Full Moon'  # example moon phase
        predicted_number = predictor.predict(recent_data, recent_zodiac, recent_moon_phase)
